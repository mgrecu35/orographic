...
def readData(fname,z1L,pRateL,z140L,pRate40L,\
    ...
    for i1,i2 in zip(a[0],a[1]):
        if piaKu[i1,i2]<40:
            ...
            z1l.append((pia2dL[-1]+np.random.randn()*2-0)/1.0)
...
def readPrecipData(fname,knn,knnC):
    ...
    for i1 in range(nx):
        for i2 in range(ny):
            if zku[i1,i2,:32].max()>0:
                z1=zku[i1,i2,:38].copy()
                z1[z1<0]=0
                z1+=np.random.randn()
                z1l=list(((z1-zm)/zs)[:32])
                z1l.append((piaKu[i1,i2]+np.random.randn()*3-0)/1.0)
                if piaKu[i1,i2]<400:
                    ijL.append([i1,i2])
                    z1L.append(z1l)
    ...
    return prate2d, pRate2D_Ret

import pickle

knnDict=pickle.load(open("knnKu.pklz","rb"))

knn=knnDict["knn"]
knnC=knnDict["knn"]
pRate2D,pRate2D_ret=readPrecipData(fnames[0],knn,knnC)
...
pickle.dump({"knn":knn},open("knnKu.pklz","wb"))
